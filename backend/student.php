...
if ($action === 'enter-code' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // MAIN FLOW: Ensure student exists in database (auto-create if needed for non-enrolled students)
    ensureStudentExists($student_id);
    
    // EXCEPTION: Check if student is excluded from course (ONLY exclusion check - enrollment NOT required)
    // Students can mark attendance even if not officially enrolled, but NOT if excluded
